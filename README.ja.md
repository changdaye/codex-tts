...
- バックグラウンド daemon ランタイムと複数セッションの focus 制御に対応
- セッション一覧と focus 切り替えのためのネイティブ macOS menubar shell を含む
...
`codex-tts` は daemon モードをサポートしています。

- `codex-tts launch -- ...` で新しい Codex セッションを起動
- daemon が launch 登録、thread バインド、rollout ポーリング、読み上げ仲裁を担当
- 同時に読み上げられるのは focus された 1 セッションだけ
- 最初に active になったセッションが自動で focus される
- 新しいセッションは既存 focus を奪わない
- focus セッションが終了したら、別セッションに自動移譲せず focus を空にする
...
従来互換も残しています。`codex-tts -- --no-alt-screen` は引き続き `codex-tts launch -- --no-alt-screen` と同じ動作です。

## Menubar Shell

リポジトリにはネイティブ macOS menubar shell も含まれています。

```text
macos/CodexTTSMenuBar
```

現時点では Swift Package として提供しています。
...
- menubar shell は現在ビルド可能なネイティブラッパーであり、署名付き配布アプリにはまだなっていない
...
- エラー通知の読み上げ追加
- 複数セッション識別の改善
- 配布用の署名済み macOS app とログイン時起動の実装
- 通知センター風の履歴パネルと最近の返信の再生
